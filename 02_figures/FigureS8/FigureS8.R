...
campos_myoInositol_timecourses <- read.csv("FigureS8_campos_myoInositol.txt", sep="\t")
ggplot(data=campos_myoInositol_timecourses, aes(x=CT, y=myoInositol, group=Campo)) +
...
campos_myoInositol <- ggplot(data=campos_myoInositol_timecourses, aes(x=CT, y=myoInositol, group=Campo)) +
...
campos_Phenylalanine_timecourses <- read.csv("FigureS8_campos_Phenylalanine.txt", sep="\t")
ggplot(data=campos_Phenylalanine_timecourses, aes(x=CT, y=Phenylalanine, group=Campo)) +
  geom_jitter(aes(col=Campo, shape = Campo),position=position_jitter(0.2), size = 3) +
  stat_smooth(aes(group = Campo, colour = Campo, fill=Campo, outfit=fit2<<-..y..),method="loess",  span = 0.5)
fit2 <- data.frame(fit2)
fit3 <- fit2[1:80, ]
...
campos_Phenylalanine <- ggplot(data=campos_Phenylalanine_timecourses, aes(x=CT, y=Phenylalanine, group=Campo)) +
  annotate("rect", xmin = -2, xmax = 0, ymin = -3, ymax = 3, alpha = .5, fill = "#e3e3e3")+
  annotate("rect", xmin = 12, xmax = 24, ymin = -3, ymax = 3, alpha = .5, fill = "#e3e3e3")+
  geom_jitter(aes(col=Campo, shape = Campo),position=position_jitter(0.2), size = 3) +
  stat_smooth(aes(group = Campo, colour = Campo, fill=Campo, outfit=fit2<<-..y..),method="loess",  span = 0.5)+
...
campos_Phenylalanine


campos_Pyruvate_timecourses <- read.csv("FigureS8_campos_Pyruvate.txt", sep="\t")
ggplot(data=campos_Pyruvate_timecourses, aes(x=CT, y=Pyruvate, group=Campo)) +
  geom_jitter(aes(col=Campo, shape = Campo),position=position_jitter(0.2), size = 3) +
  stat_smooth(aes(group = Campo, colour = Campo, fill=Campo, outfit=fit2<<-..y..),method="loess",  span = 0.5)
fit2 <- data.frame(fit2)
fit3 <- fit2[1:80, ]
...
campos_Pyruvate <- ggplot(data=campos_Pyruvate_timecourses, aes(x=CT, y=Pyruvate, group=Campo)) +
  annotate("rect", xmin = -2, xmax = 0, ymin = -3, ymax = 3, alpha = .5, fill = "#e3e3e3")+
  annotate("rect", xmin = 12, xmax = 24, ymin = -3, ymax = 3, alpha = .5, fill = "#e3e3e3")+
  geom_jitter(aes(col=Campo, shape = Campo),position=position_jitter(0.2), size = 3) +
  stat_smooth(aes(group = Campo, colour = Campo, fill=Campo, outfit=fit2<<-..y..),method="loess",  span = 0.5)+
...
plot_grid(campos_myoInositol, campos_Phenylalanine, campos_Pyruvate, labels = c("A","B","C"), ncol = 3, align = "none", rel_widths = c(1.2, 1, 1),rel_heights = c(1),label_size = 20)